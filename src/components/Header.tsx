...
const Header = ({ contentsByCategory }) => {
  ...
  useEffect(() => {
    ...
    // $activeTitle
    // $activeItem
    // const testObj = contentsByCategory.map((content) => {
    //   return {
    //     [content.name]: content.name === $activeTitle ? true : false,
    //   }
    // })

    // initial expanded sections setup
    const initialExpandedSections = contentsByCategory.reduce((acc, { name }) => {
      acc[name] = false
      return acc
    }, {})
